...
questions = [
    ...
    "Based on the corpus, which microstructural feature — grain size, precipitate distribution, or phase composition — has the strongest influence on yield strength across all alloy systems?",
    "If you had to design an alloy for simultaneous high strength, corrosion resistance, and high temperature stability, what composition and processing route would the corpus evidence support?",
    "How do the strengthening contributions of solid solution hardening, precipitation hardening, and grain boundary strengthening compare quantitatively across the alloy systems in the corpus?",
...
with open("test_results.txt", "w", encoding="utf-8") as output_file:
    ...
    for i, q in enumerate(questions):
        ...
        if result is None:
            output_file.write("Fell back to web.\n\n")
            fallback.append((i + 1, q))
        else:
        ...
        summary = (
            ...
            f"SUMMARY\n"
            f"{'='*60}\n"
            f"Answered from corpus: {len(passed)}\n"
            f"Fell back to web: {len(fallback)}\n\n"
        )
